...
const ACCENT_RGB = '249, 115, 22'; // orange-500
const ACCENT_HEX = '#f97316';

const numFields: { id: string; label: string; field: keyof ThyroidInputs; placeholder: string; step?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>['inputMode'] }[] = [
  { id: 'thAge', label: 'Age', field: 'age', placeholder: 'e.g. 35' },
  { id: 'tsh', label: 'TSH (uIU/mL)', field: 'tsh', placeholder: 'e.g. 2.5', step: '0.01', inputMode: 'decimal' },
  { id: 'ft3', label: 'Free T3 (pg/mL)', field: 'freeT3', placeholder: 'e.g. 3.0', step: '0.1', inputMode: 'decimal' },
  { id: 'ft4', label: 'Free T4 (ng/dL)', field: 'freeT4', placeholder: 'e.g. 1.2', step: '0.1', inputMode: 'decimal' },
];
...
const ThyroidAssessment: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logPrediction('thyroid', validationResult.data, finalResult);
    } catch (err: any) {
      console.error(err);
      const offlineFacts = getLocalPredictionFallback('thyroid', validationResult.data);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <motion.div 
            className="w-12 h-12 rounded-xl flex items-center justify-center bg-orange-500/15 shrink-0"
        ...
        >
          ...
          >
            ...
            <motion.div className="space-y-1.5" variants={fieldVariant}>
              <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400" htmlFor="thGender">Gender</label>
            ...
            {/* Weight Change */}
            <motion.div className="space-y-1.5" variants={fieldVariant}>
              <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400" htmlFor="wtChange">Weight Change</label>
              <div className="relative">
                <select
                  id="wtChange"
                  className="predictor-input w-full h-12 rounded-xl px-4 py-3 text-sm text-slate-800 dark:text-white font-medium outline-none appearance-none pr-10 cursor-pointer"
            ...
            <motion.div className="space-y-1.5" variants={fieldVariant}>
              <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400" htmlFor="tempSens">Temperature Sensitivity</label>
              <div className="relative">
                <select
                  id="tempSens"
                  className="predictor-input w-full h-12 rounded-xl px-4 py-3 text-sm text-slate-800 dark:text-white font-medium outline-none appearance-none pr-10 cursor-pointer"
            ...
            <motion.div className="sm:col-span-2 border-t border-slate-200 dark:border-slate-800 pt-4 space-y-3" variants={fieldVariant}>
              <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 block uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-orange-500/60" />Select Symptomatic Indicators
              </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  const isActive = !!formData[item.key as keyof ThyroidInputs];
                  return (
                    <motion.button
                      type="button"
                      key={item.key}
                      ...
                      className={`flex items-center justify-between p-3.5 border rounded-xl text-left cursor-pointer h-12 transition-all duration-300 ${
                        isActive
                          ? 'predictor-checkbox-active border-orange-500/40 bg-orange-500/8 dark:bg-orange-500/10'
                          : 'border-slate-250 dark:border-slate-800 hover:border-orange-500/25 hover:bg-orange-500/3'
                      }`}
                      style={{ '--accent-rgb': ACCENT_RGB } as React.CSSProperties}
                    >
                      <span className={`text-xs font-semibold leading-tight transition-colors duration-200 ${
                        isActive ? 'text-orange-700 dark:text-orange-300' : 'text-slate-700 dark:text-slate-200'
                      }`}>
                        {item.label}
                      </span>
                      <div 
            ...
            >
              ...
              {loading ? (
                <>
                  ...
                  <span className="relative z-10">Analyzing thyroid function...</span>
                </>
              ) : (
                <>
                  <Send className="w-4 h-4 relative z-10" />
                  <span className="relative z-10">Run Thyroid Assessment</span>
                </>
              )}
            </motion.button>
          </div>
          ...
          <AnimatePresence mode="wait">
            {result ? (
              ...
              >
                ...
                <motion.div 
                  className="w-16 h-16 rounded-2xl flex items-center justify-center relative z-10 bg-orange-500/12"
                  animate={{ y: [0, -6, 0] }}
                  transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
                >
